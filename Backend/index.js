  ...
  try {
    ...
    for (let i = 0; i < search_arr.length; i++) {
      for (let j = 0; j < data.length; j++) {
        if (search_arr[i].toUpperCase() == data[j].city.toUpperCase()) {
        ...
        function count(plc) {
          return plc.trim().split(/[,\s]+/).filter(word => word);
        }
        let place_arr = count(plc)
        for (let k = 0; k < place_arr.length; k++) {
          if (search_arr[i].toUpperCase() == place_arr[k].toUpperCase()) {
            ; // Extract YYYY-MM-DD
...
app.post("/api/search_by_jatra_id", async function (req, res) {
...
app.post('/api/razorpay', async (req, res) => {
  ...
  //api for razorpay....
  res.json({ message: "creat order id", id: order })
...
app.post('/api/booking', async (req, res) => {
  ...
  let razorpay_Id = req.body.razorpayPaymentId
  let pay_Signature = req.body.razorpaySignature
  console.log(req.body)

  /*razorpay virification.........*/

  async function verifyPayment(razorpayOrderId, razorpayPaymentId, razorpaySignature) {
    try {
      const crypto = require('crypto');
      const shasum = crypto.createHmac('sha256', process.env.YOUR_RAZORPAY_TEST_KEY_SECRET);
  ...
  async function main() {
    try {
      ...
      // Simulate payment data (replace with actual client-side data)
      const razorpayOrderId = pay_id;
      const razorpayPaymentId = razorpay_Id; // Replace with real payment ID
      const razorpaySignature = pay_Signature; // Replace with real signature
      ...
      if (isPaymentValid) {
        ...
        const { data, error } = await supabase
          .from('booking')
          .insert([{ u_id: u_id, jatra_id: jatra_id, seats: seats, pay_id: pay_id, amount: p_amount }])
          .select('*')
          ...
          if (!error) {
            ...
            try {
              ...
              const { data, error } = await supabase
                ...
                .eq('id',jatra_id)
                console.log(data)
            //console.log(data[0].booked.length)
           if (data.length == 0) {
              const { data, error } = await supabase
                .from('booked_seats')
                .insert([{ id:jatra_id, booked: r }])
              ...
              try { 
                ...
                // console.log( order)
                console.log("data inshort successfuly");
              } catch (error) {
            ...
            console.error(error);
            res.status(500).send('Error inshort in booking_seats');
          }
        } else {
          console.log(error)
        }
    ...
    if (data!=null) {
      if (p.length != 0) {
        for (let i = 0; i < p.length; i++) {
          ...
          p[i].date = data[0].date
          p[i].time = data[0].time
          p[i].poster = data[0].poster
...
app.post("/api/party/jatra_id", async function (req, res) {
  ...
  res.json({ message: 'not found', data: data });
})
